...
% - Functions for reading neuroimaging data (e.g., NIfTI files). A placeholder
...
[~, schaefer_atlas] = your_nifti_read_function(path_schaefer_atlas);
% Load a network mask (e.g., Yeo 17) to constrain the atlas.
[~, yeo_networks] = your_nifti_read_function(path_yeo_networks);
...
[~, roi_parcels] = your_nifti_read_function(path_roi_parcellation);
...
[~, roi_left_mask] = your_nifti_read_function(path_roi_left_mask);
ind_roi_left_voxels = find(roi_left_mask ~= 0);

[~, roi_right_mask] = your_nifti_read_function(path_roi_right_mask);
...
for i = 3:Length_Names
    ...
    load(fullfile(path_functional_data, subject_id, 'GPL_TS.mat'), 'GPL_TS'); % Left ROI
    load(fullfile(path_functional_data, subject_id, 'GPR_TS.mat'), 'GPR_TS'); % Right ROI
    load(fullfile(path_functional_data, subject_id, 'net_ts_avg_400.mat'), 'net_ts_avg_400'); % Cortical parcels
